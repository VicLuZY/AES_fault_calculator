...
pub fn sample_network() -> Network {
    ...
    n.add_bus("msb_480", "Main Switchboard", 0.48, 445.0, 245.0).unwrap();
    n.add_bus("mcc_480", "Mechanical MCC", 0.48, 745.0, 245.0).unwrap();
    n.add_utility_source("src_utility", "Utility source equivalent", "util_12k", 500.0, 10.0, 1.0).unwrap();
    n.add_transformer("tx_1", "2500 kVA transformer", "util_12k", "msb_480", 2500.0, 0.48, 5.75, 7.0, 5.75, true).unwrap();
    n.add_branch(Branch {
        id: "fdr_mcc".to_string(),
        kind: "feeder".to_string(),
        name: "MSB to MCC feeder".to_string(),
        from: "msb_480".to_string(),
        to: "mcc_480".to_string(),
